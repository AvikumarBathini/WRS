        ...
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            if (context.InputParameters.Contains("Target") && context.InputParameters["Target"] is EntityReference)
            {
                ...
                try
                {
                    ...
                    bool isPublished = _product.Contains("wrs_published") ? _product.GetAttributeValue<bool>("wrs_published") : false;
                    ...
                    {
                        int productstructure = -1;
                        Entity _tepmProduct = service.Retrieve(targetEntity.LogicalName, targetEntity.Id, new ColumnSet("productstructure"));
                        if (_tepmProduct.Contains("productstructure"))
                            productstructure = _tepmProduct.GetAttributeValue<OptionSetValue>("productstructure").Value;
                        ...
                        if (productObj.IsSuccess && isPublished)
                        {
                            ...
                            if (productstructure == 2)
                            {
                                ...
                                {
                                    Target = new EntityReference(_tepmProduct.LogicalName, _tepmProduct.Id)
                                };
                                service.Execute(Req);
                            }
                            else
                        ...
                        {
                            Entity product = new Entity(targetEntity.LogicalName, targetEntity.Id);
                            product.Attributes.Add("statecode", new OptionSetValue(2));
                            product.Attributes.Add("statuscode", new OptionSetValue(167320000));
                            service.Update(product);
                        }
                    }
                }
