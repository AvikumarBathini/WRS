...
function emailOnLoad(executionContext) {
	...
	if ((formType == FORM_TYPE_CREATE || formType == FORM_TYPE_UPDATE) && fromSet != true) {
	    ...
	    retrieveUserTeams(executionContext, getLoggedinUser());
		if (isUserPresentinGETeam) {
			...
			formContext.getAttribute("from").setValue(enquiryQueue);
			fromSet = true;
		}
		if (isUserPresentinMembershipTeam) {
			...
			formContext.getAttribute("from").setValue(membershipQueue);
			fromSet = true;
		}
	}
}
...
function retrieveUserTeams(userId) {
	...
	SDK.WEBAPI.retrieveMultipleRecordsSync("teammemberships", query, function (result) {
		...
		if (result.value.length > 0) {
		    ...
		    Xrm.Navigation.openAlertDialog("Escalate to TeamID not found");
		}
		//alert("Success");
	},
		function () {
		    aleXrm.Navigation.openAlertDialogrt("Error extracing Escalate to TeamID");
		return;
	},
		function () {
		//alert("Complete");
...
function retrieveTeamName(teamid) {
	...
	SDK.WEBAPI.retrieveMultipleRecordsSync("teams", query, function (result) {
		...
		function () {
		    Xrm.Navigation.openAlertDialog("Error extracing Escalate to TeamID");
		return;
	},
		function () {
		//alert("Complete");
...
function getQueueId(enquiryQueueName) {
	...
	SDK.WEBAPI.retrieveMultipleRecordsSync("queue", query, function (result) {
		...
		if (result.value.length > 0) {
		    ...
		    Xrm.Navigation.openAlertDialog("queueid not found");
		}
		//alert("Success");
	},
		function () {
		    Xrm.Navigation.openAlertDialog("Error extracing queueid");
		return;
	},
		function () {
		//alert("Complete");
...
SDK.WEBAPI = {
	...
	retrieveMultipleRecordsSync: function (type, options, successCallback, errorCallback, OnComplete) {
	    ...
	    req.open("GET", Xrm.Utility.getGlobalContext().getClientUrl() + "/api/data/v8.2/" + type + optionsString, false);
