        ...
        public Entity PrepareOrderItemObject(DataRow dr)
        {
            ...
            if (dr["Id"] != DBNull.Value)
            {
                ...
                if (dr["UnitPriceInclTax"] != DBNull.Value && decimal.Parse(dr["UnitPriceInclTax"].ToString()) > 0)
                    orderitemToBeCreatedFromDB.wrs_unitpriceincltax = new Money(decimal.Parse(dr["UnitPriceInclTax"].ToString()));
                if (dr["UnitPriceExclTax"] != DBNull.Value && decimal.Parse(dr["UnitPriceExclTax"].ToString()) > 0)
                    orderitemToBeCreatedFromDB.wrs_unitpriceexcltax = new Money(decimal.Parse(dr["UnitPriceExclTax"].ToString()));
                if (dr["PriceInclTax"] != DBNull.Value && decimal.Parse(dr["PriceInclTax"].ToString()) > 0)
                    orderitemToBeCreatedFromDB.wrs_priceincltax = new Money(decimal.Parse(dr["PriceInclTax"].ToString()));
                if (dr["PriceExclTax"] != DBNull.Value && decimal.Parse(dr["PriceExclTax"].ToString()) > 0)
                    orderitemToBeCreatedFromDB.wrs_priceexcltax = new Money(decimal.Parse(dr["PriceExclTax"].ToString()));
