    ...
    class Program
    {
        ...
        private Entity PreparePriceListItemObject(DataRow dr, bool isBasePriceListItem = false)
        {
                ...
                dr["ProductTypeId"] != DBNull.Value && int.Parse(dr["ProductTypeId"].ToString()) == 5)
            {
                ...
                {
                    Guid priceGroupGuid = GetDeterministicGuid(dr["Id"].ToString());
                    PriceGroupLists.Add(new PriceGroup()
                    {
                        Guid = new Guid(priceGroupGuid.ToString().ToUpper()),
                        Id = int.Parse(dr["Id"].ToString()),
                        ProductId = int.Parse(dr["ProductId"].ToString()),
                        PriceGroupId = int.Parse(dr["PriceGroupId"].ToString())
                    });
                    priceListItem.ProductPriceLevelId = new Guid(priceGroupGuid.ToString().ToUpper());
            ...
            try
            {
                ...
                foreach (ExecuteMultipleResponseItem e in emrResponse.Responses)
                {
                    if (e.Fault == null)
                    {
                        ...
                        //else if (isPriceListItemUpdate)
                        //    ((EntityReference)((UpdateRequest)(multipleReq.Requests[e.RequestIndex])).Target.Attributes["pricelevelid"]).KeyAttributes.TryGetValue("wrs_id", out t);
                        else if (((multipleReq.Requests[e.RequestIndex])).RequestName.ToLower() == "upsert")
                    ...
                    else if (e.Fault != null)
                    {
                        ...
                        if (isPriceListItemCreate)
                        {
                            ...
                            if (!e.Fault.Message.ToString().Contains("This product and unit combination has a price for this price list"))
                        ...
                        if (isPriceListItemUpdate)
                        {
                            //((EntityReference)((CreateRequest)(multipleReq.Requests[e.RequestIndex])).Target.Attributes["objectId"]).KeyAttributes.TryGetValue("wrs_id", out t1);
                            ((EntityReference)((UpdateRequest)(multipleReq.Requests[e.RequestIndex])).Target.Attributes["productid"]).KeyAttributes.TryGetValue("wrs_id", out t1);
                            ((EntityReference)((UpdateRequest)(multipleReq.Requests[e.RequestIndex])).Target.Attributes["pricelevelid"]).KeyAttributes.TryGetValue("wrs_id", out t2);
                            NCId = t2.ToString();
                            PriceGroupLists.Remove(PriceGroupLists.Where(g => g.PriceGroupId == int.Parse(t2.ToString()) && g.ProductId == int.Parse(t1.ToString())).FirstOrDefault());
                        }
                        else
