...
namespace BlackoutCalendarImport
{
    class Program
    {
        ...
        private string RetreveAlternateKey(Guid iD, string entityLogicalName, string alternate, string primary)
        {
            QueryExpression accountbyID = new QueryExpression(entityLogicalName);
            accountbyID.Criteria.AddCondition(new ConditionExpression(primary, ConditionOperator.Equal, iD));
            accountbyID.ColumnSet.Columns.Add(alternate);
            Entity e = serviceClient().Retrieve(entityLogicalName, iD, accountbyID.ColumnSet);
            if (e != null && e.Attributes[alternate] != null && !entityLogicalName.Equals(TransactionCurrency.EntityLogicalName))
